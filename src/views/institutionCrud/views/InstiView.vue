<template>
    <div class="card w-full bg-gray-100 flex items-center justify-center">
        <div class="w-full">
            <h2 class="text-2xl font-bold mb-6">Mantenimiento de institucion</h2>
            <CModal v-model:visible="modalVisibleC" />
            <EModal v-model:visible="modalVisibleE" />

            <div class="flex flex-wrap gap-2 p-5">
                <Button label="Nueva institucion" icon="pi pi-plus" @click="abrirCrear" />
                <Button label="Eliminar" icon="pi pi-trash" severity="danger" />
            </div>
            <div class="overflow-x-auto">
                <DataTable :value="institucionVisibles" v-model:selection="seleccionados" selectionMode="single" dataKey="id" :rowHover="true" class="w-full">
                    <Column selectionMode="multiple" headerStyle="width: 3rem" />
                    <Column field="name_institution" header="Nombre institucion" />
                    <Column header="Acciones">
                        <template #body="{ data }">
                            <Button label="editar" icon="pi pi-pencil" class="p-button-sm mr-2" @click="abrirEditar(data)" severity="success" />
                            <!-- <Button label="borrar" icon="pi pi-trash" class="p-button-sm p-button-danger" @click="confirmarEliminar(data)" /> -->
                        </template>
                    </Column>
                </DataTable>
            </div>
        </div>
    </div>
</template>

<script setup>
import { ref } from 'vue';

import Button from 'primevue/button';
import Column from 'primevue/column';
import DataTable from 'primevue/datatable';
import CModal from '../components/CmodalI.vue';
import EModal from '../components/EmodalI.vue';
// import FModal from '../components/FModalCU.vue'; // Si necesitas el filtro, descomentar esta línea

// Datos simulados
const institucion = ref([
    { id: 1, name_institution: 'Institución A' },
    { id: 2, name_institution: 'Institución B' },
    { id: 3, name_institution: 'Institución C' },
    { id: 4, name_institution: 'Institución D' },
    { id: 5, name_institution: 'Institución E' }
]);
const institucionVisibles = ref(institucion);

const modalVisibleC = ref(false);
const modalVisibleE = ref(false);
const seleccionados = ref([]);
const modoEdicion = ref(false);
const institucionEditando = ref(null);

const formulario = ref({
    name_institution: ''
});

const abrirCrear = () => {
    modoEdicion.value = false;
    institucionEditando.value = null;
    formulario.value = {
        name_institution: ''
    };
    modalVisibleC.value = true;
};

const abrirEditar = (inst) => {
    modoEdicion.value = true;
    institucionEditando.value = inst;
    Object.assign(formulario.value, inst);
    modalVisibleE.value = true;
};

// const confirmarEliminar = (usuario) => {
//   usuarios.value = usuarios.value.filter(u => u.id !== usuario.id);
// };
</script>
